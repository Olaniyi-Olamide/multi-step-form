...
function Plans() {
  const { monthly, plans, currentPlan, setCurrentPlan } = useForm();
  ...
  return (
    <section className="flex flex-col gap-[1rem] basis-[70%] px-[2rem] bg-Blue100 mx-[1.2rem] md:mx-0 py-[2rem] rounded-2xl mt-[-5rem] md:mt-0 mb-[2rem] md:mb-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-[1.5rem]">
        {plans.map(
          (plan: { id: number; icon: string; plan: string; price: string }) => {
            return (
              <figure
                key={plan.id}
                className={`flex md:flex-col rounded-lg px-[0.8rem] py-[1rem] border-[0.12rem] border-Grey500 gap-[2rem] lg:w-[20%] ${
                  currentPlan?.id === plan.id ? "active-plan" : ""
                }`}
              ...
              >
                ...
                <figcaption>
                  <h5 className="font-bold text-Blue950 text-[0.85rem]">
                    {plan.plan}
                  </h5>
                  <p className="text-[0.75rem] text-Grey500">{plan.price}</p>
                  {monthly && (
